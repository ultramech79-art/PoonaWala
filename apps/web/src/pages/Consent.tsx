      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-5 pb-6">
            ...
            {HOW_IT_WORKS.map(({ icon: Icon, title, desc }, i) => (
              <div key={title} className="card p-4">
                <div className="w-10 h-10 rounded-2xl bg-brand-50 flex items-center justify-center mb-3">
                  <Icon className="w-5 h-5 text-brand-500" strokeWidth={1.8} />
                </div>
        ...
        <div className="card p-5 mb-5">
          <div className="flex items-center gap-2.5 mb-4">
            <ShieldCheck className="w-5 h-5 text-brand-500" strokeWidth={2} />
        ...
        <button
          ...
          className="w-full flex items-start gap-3 p-4 rounded-2xl border transition-all duration-200 mb-6 text-left"
          style={{ borderColor: agreed ? '#5B47FA' : '#E5E5E5', background: agreed ? '#F0EEFF' : 'white' }}
        >
          <div className={`w-5 h-5 rounded-md flex-shrink-0 mt-0.5 flex items-center justify-center transition-all ${agreed ? 'bg-brand-500' : 'bg-white border-2 border-stone-300'}`}>
